...
function AuthGateInner({ children, fallbackKey, fallbackDescKey, fallbackMessage, fallbackDescription }: AuthGateProps) {
  ...
  const socialAuthEnabled = false // TODO: enable after Google/Apple provider configuration.
      ...
      <div className="flex items-center justify-center min-h-[60vh] px-4">
        <Card className="max-w-md w-full text-center border-border shadow-lg">
          ...
          <CardContent className="space-y-3">
            {!usePassword && sent ? (
              ...
              <form onSubmit={handleMagicLink} className="space-y-3">
                ...
                <Button type="submit" size="lg" disabled={sending || !email.trim()} className="w-full gap-2 font-medium">
                  {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />}
                  {sending ? t('auth.sending') : t(authMode === 'signUp' ? 'auth.signUpCta' : 'auth.signInCta')}
                </Button>
              </form>
            )}
            <button
            ...
            {socialAuthEnabled && (
              <div className="relative my-1"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-card px-2 text-muted-foreground">{t('auth.orContinueWith')}</span></div></div>
            )}
