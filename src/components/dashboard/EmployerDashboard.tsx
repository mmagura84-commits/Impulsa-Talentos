...
export function EmployerDashboard({ employerId }: { employerId: string }) {
  ...
  return <div className="space-y-6">
    <div className="grid sm:grid-cols-3 gap-4"><StatCard icon={Briefcase} label={t('dashboard.stat.activeJobs')} value={String(postedJobs.filter(j=>j.status==='open').length)} delay={0} accent="navy"/><StatCard icon={Users} label={t('dashboard.stat.totalApplicants')} value={String(applications.length)} delay={0.05} accent="navy"/><StatCard icon={CheckCircle2} label={t('dashboard.stat.hires')} value={String(hires)} delay={0.1} accent="navy"/></div>
