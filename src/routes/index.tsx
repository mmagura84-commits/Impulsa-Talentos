...
function JobFeedPreview() {
  ...
  const card = (job: Job) => {
    ...
    const salaryMin = typeof job.salaryMin === 'number' && Number.isFinite(job.salaryMin) ? job.salaryMin : null
    const salaryMax = typeof job.salaryMax === 'number' && Number.isFinite(job.salaryMax) ? job.salaryMax : null
    const salary = salaryMin === null && salaryMax === null
      ? null
      : job.currency === 'USD'
        ? `${salaryMin?.toLocaleString() ?? '?'}–${salaryMax?.toLocaleString() ?? '?'}`
        : `${job.currency} ${salaryMin?.toLocaleString() ?? '?'}–${salaryMax?.toLocaleString() ?? '?'}`
