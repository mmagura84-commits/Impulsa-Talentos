...
function EmployerAnalytics() {
 ...
 const stats=[['analytics.activeJobs',String((jobs??[]).filter(j=>j.status==='open').length),Briefcase],['analytics.totalApplicants',String(all.length),Users],['analytics.interviews',String(all.filter(a=>a.status==='interview').length),BarChart3],['analytics.hires',String(all.filter(a=>a.status==='hired').length),CheckCircle2]] as const
